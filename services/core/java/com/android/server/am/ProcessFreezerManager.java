...
public class ProcessFreezerManager {
    ...
    private boolean isBoundClient(ProcessServiceRecord psr, String processName, boolean equal) {
    ...
    public boolean checkNeedFreezeProcessLocked(ProcessRecordInternal app) {
        ...
        synchronized (mPackagesSelfLocked) {
            for (String packageName : mPackagesSelfLocked.mPackageMap.keySet()) {
                ...
                final ProcessServiceRecord psr = findProcessByNameLocked(app.processName).mServices;
                if (psr != null && isBoundClient(psr, packageName, true)) {
                    isInList = true;
                }
            }
            return isInList;
